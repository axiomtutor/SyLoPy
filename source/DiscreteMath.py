...
Relation syntax and semantics live behind a theory-level declaration recipe.
The historical implementation is retained in ``DiscreteMathLegacy`` while
this module supplies the public theory environment and its extensibility
boundary.
"""
...
from SyLoPy.source import DiscreteMathLegacy as _legacy
from SyLoPy.source.DiscreteMathLegacy import *  # noqa: F401,F403
...
class RelationDeclarationRecipe:
    ...
    _properties = ("reflexive", "irreflexive", "symmetric", "antisymmetric", "asymmetric", "transitive", "total")
    ...
    def try_match(self, clauses, index):
        ...
        if "relation" not in descriptor and not any(alias in descriptor for alias in self._aliases):
            return None
        carrier_match = re.search(r"\bon\s+([A-Za-z_][A-Za-z0-9_]*)\s*$", getattr(clause, "descriptor", ""), re.I)
        ...
        declarations = [
            pl.Declaration(name=name, kind=pl.DeclarationKind.PREDICATE, arity=2,
                           type_name=getattr(clause, "descriptor", "relation"), metadata=metadata)
...
DISCRETE_MATH_ENVIRONMENT = _legacy.DISCRETE_MATH_ENVIRONMENT.extended(
