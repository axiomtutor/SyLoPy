...
class _ElaborationContext:
    ...
    def _elaborate_entry_impl(self, entry: Any) -> Any:
        if isinstance(entry, SurfaceSubproof):
            body = [self.elaborate_entry(item) for item in entry.entries]
            if entry.label:
        ...
        try:
            ...
            if tag == 'rule_below':
                ...
                nested = [self.elaborate_entry(item) for item in entry.subproofs[0].entries]
            ...
            if tag == 'rule_hybrid':
                ...
                nested = [
                    [self.elaborate_entry(item) for item in subproof.entries]
